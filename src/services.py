...
def df_to_dict(df: DataFrame) -> list[dict[str, Any]]:
...
def cashback_analysis(data: list[dict], year: int, month: int):
    ...
    if not sorted_by_category:
        ...
        return json.dumps({'message': f'За {month}.{year} кэшбэка нет'}, ensure_ascii=False)
