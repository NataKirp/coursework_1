...
def spending_by_category(
    ...
    if filtered_by_date is None or filtered_by_date.empty:
        ...
        return json.dumps(
            {"message": f"За период c {start_date} по {end_date} нет данных по тратам"},
            ensure_ascii=False,
        )
    ...
    if filtered_by_category is None or filtered_by_category.empty:
        ...
        return json.dumps(
            {
                "message": f"За период c {start_date} по {end_date} нет трат по категории {category}"
            },
            ensure_ascii=False,
        )

    grouped = (
        filtered_by_category.groupby("Описание")["Сумма операции"].sum().reset_index()
    )
    grouped = grouped.sort_values(
        by="Сумма операции", ascending=True
    )  # True, т.к. суммы отрицательные, по убыванию
